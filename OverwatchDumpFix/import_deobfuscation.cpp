...
)
{
    ...
    *pDeobfuscatedEntry = NULL;
        ...
        /**/)
    {
        ...
        count = cs_disasm(handle, (unsigned char*)pInstruction, 0x20, (uint64_t)pInstruction, 0, &insn);


        if (count > 0)
        {
            if (strstr(insn->op_str, "rax"))
            {
                //mov
                if (strstr(insn->mnemonic, "mov"))
                    Rax = insn->detail->x86.operands[1].imm;
                //sub
                else if (strstr(insn->mnemonic, "sub"))
                    Rax -= insn->detail->x86.operands[1].imm;
                //add
                else if (strstr(insn->mnemonic, "add"))
                    Rax += insn->detail->x86.operands[1].imm;
                //xor
                else if (strstr(insn->mnemonic, "xor"))
                    Rax ^= insn->detail->x86.operands[1].imm;
                else if (strstr(insn->mnemonic, "imul"))
                {
                    if ((insn->detail->x86.operands->reg == X86_REG_RAX))
                    {
                        Rax *= r10;
                    }
                    else//imul r10,rax
                ...
                else if (strstr(insn->mnemonic, "jmp"))
                    ;
                else 
                    DebugBreak();
            }
            else if (strstr(insn->op_str, "r10"))
            {
                //mov
                if (strstr(insn->mnemonic, "mov"))
                    r10 = insn->detail->x86.operands[1].imm;
                //sub
                else if (strstr(insn->mnemonic, "sub"))
                    r10 -= insn->detail->x86.operands[1].imm;
                //add
        ...
        case INSN_OPCODE_MOV_R64_IMM64_RAX_R10:
            ...
            //r10 = insn->detail->x86.operands->imm;
            break;

        case INSN_OPCODE_MOV_R64_IMM64://mov rax,xx 
            //Rax = insn->detail->x86.operands->imm;
            break;

        case INSN_OPCODE_ADD_R64_IMM32://add rax,xx
            //Rax += insn->detail->x86.operands->imm;
            break;

        case INSN_OPCODE_SUB_R64_IMM32://sub rax,xxx
            //Rax -= Disassembly.imm.imm32;
            //Rax -= insn->detail->x86.operands->imm;
            break;
        ...
        case INSN_OPCODE_JMP_REG:
            DeobfuscatedEntry = Rax;
            //return false;//µ•¥Œ≤‚ ‘
            plog("Excute after inst RAX %llx\n", Rax);
            break;
        ...
        //plog("Excute after inst RAX R10 %llx %llx\n", Rax, r10);
        //
